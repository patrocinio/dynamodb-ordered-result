...
public class Handler {
    ...
    public static void putItemInTable(DynamoDbAsyncClient ddb,
            String tableName,
            String key,
            String keyVal,
            String seqNo,
            String seqNoVal) 
    {
        ...
        try {
            CompletableFuture<PutItemResponse> response = ddb.putItem(request);
            logger.info(tableName + " was successfully updated. The request id is "
                + response.get().responseMetadata().requestId());
    ...
    public static QueryResponse getItemFromTable(DynamoDbAsyncClient ddb,
            String tableName,
            String key,
            String keyVal) 
    {
...
//        logger.info ("request: " + request.toString());
        ...
        try {
            CompletableFuture<QueryResponse> response = ddb.query(request);
            return response.get();
...
//        logger.info("ReadRequest map: " + map);
    ...
    public static void trimTable() {
        ...
        CompletableFuture<ScanResponse> response;
...
//            logger.info ("trimTable response: " + response);
                ...
                for (Map<String, AttributeValue> item : response.get().items()) {
